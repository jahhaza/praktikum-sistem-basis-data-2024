...
SELECT pelanggan.nama_pelanggan, 
       GROUP_CONCAT(kamar.nama_kamar ORDER BY kamar.nama_kamar) AS nama_kamar, 
       COUNT(kamar.nomor) AS jumlah_kamar
FROM `order`
JOIN pelanggan ON `order`.pelanggan_NIK = pelanggan.NIK
JOIN order_kamar ON `order`.id = order_kamar.order_id
JOIN kamar ON order_kamar.kamar_nomor = kamar.nomor
WHERE pelanggan.nama_pelanggan = 'Daniel Martinez'
GROUP BY pelanggan.nama_pelanggan;

-- 3 --
SELECT resepsionis.nama_resepsionis, COUNT(`order`.id) AS jumlah_order
FROM resepsionis
JOIN `order` ON resepsionis.id = `order`.resepsionis_id
JOIN order_kamar ON `order`.id = order_kamar.order_id
GROUP BY resepsionis.id
HAVING COUNT(order_kamar.kamar_nomor) > 2;
...
-- 5 --
SELECT kamar.nomor, kamar.nama_kamar, kamar.tipe_kamar
FROM kamar
JOIN kamar_fasilitas ON kamar.nomor = kamar_fasilitas.kamar_nomor
JOIN fasilitas ON kamar_fasilitas.fasilitas_id = fasilitas.id
WHERE fasilitas.nama = 'Wi-Fi';

-- 6 --
SELECT SUM(DATEDIFF(`order`.tanggal_checkout, `order`.tanggal_checkin) * kamar.harga_per_malam) AS total_pendapatan,
       AVG(DATEDIFF(`order`.tanggal_checkout, `order`.tanggal_checkin) * kamar.harga_per_malam) AS rata_rata_pendapatan
FROM `order`
JOIN order_kamar ON `order`.id = order_kamar.order_id
JOIN kamar ON order_kamar.kamar_nomor = kamar.nomor;

-- 7 --
SELECT pelanggan.NIK, pelanggan.nama_pelanggan, pelanggan.no_telp,
       kamar.nomor AS kamar_nomor, kamar.nama_kamar, kamar.tipe_kamar
FROM pelanggan
JOIN `order` ON pelanggan.NIK = `order`.pelanggan_NIK
JOIN pembayaran ON `order`.id = pembayaran.order_id
JOIN order_kamar ON `order`.id = order_kamar.order_id
JOIN kamar ON order_kamar.kamar_nomor = kamar.nomor
WHERE pembayaran.status != 'Lunas';

-- 8 --
SELECT kamar.nomor, kamar.nama_kamar, 
       GROUP_CONCAT(fasilitas.nama ORDER BY fasilitas.nama) AS fasilitas
FROM kamar
JOIN kamar_fasilitas ON kamar.nomor = kamar_fasilitas.kamar_nomor
JOIN fasilitas ON kamar_fasilitas.fasilitas_id = fasilitas.id
WHERE kamar.tipe_kamar = 'Deluxe'
GROUP BY kamar.nomor;

-- 9 --
SELECT fasilitas.nama, COUNT(kamar_fasilitas.fasilitas_id) 
AS jumlah_penggunaan
FROM fasilitas
JOIN kamar_fasilitas ON fasilitas.id = kamar_fasilitas.fasilitas_id
