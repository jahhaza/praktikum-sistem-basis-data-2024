...
SELECT DISTINCT A.A_NRP, A.A_Nama
FROM Alumni A
JOIN Pendataan P ON A.A_NRP = P.Alumni_A_NRP
JOIN Pendataan_Kegiatan PK ON P.PD_ID = PK.Pendataan_PD_ID
...
FROM Admin A
JOIN Pendataan P ON A.P_NIK = P.Admin_P_NIK
WHERE P.PD_ID BETWEEN 'PD0010' AND 'PD0015';

SELECT DISTINCT A.P_NIK, P.PD_ID
FROM Admin A
JOIN Pendataan P ON A.P_NIK = P.Admin_P_NIK
JOIN Alumni AL ON P.Alumni_A_NRP = AL.A_NRP
WHERE AL.A_Pekerjaan = 'Dosen';

SELECT A.A_NRP, A.A_Nama, COUNT(*) AS Jumlah_Kegiatan
FROM Alumni A
JOIN Pendataan P ON A.A_NRP = P.Alumni_A_NRP
JOIN Pendataan_Kegiatan PK ON P.PD_ID = PK.Pendataan_PD_ID
GROUP BY A.A_NRP, A.A_Nama
HAVING COUNT(*) >= 2;

SELECT DISTINCT A.P_NIK, A.P_Nama
FROM Admin A
JOIN Pendataan P ON A.P_NIK = P.Admin_P_NIK
JOIN Alumni AL ON P.Alumni_A_NRP = AL.A_NRP
WHERE AL.A_Pekerjaan = (
    SELECT A_Pekerjaan
    FROM Alumni
    WHERE A_Nama = 'Restri Amalia'
) AND AL.A_Nama != 'Restri Amalia';

SELECT A_Pekerjaan, COUNT(*) AS Jumlah_Alumni
FROM Alumni
WHERE A_Pekerjaan IS NOT NULL
GROUP BY A_Pekerjaan
...
SELECT A.P_Nama, COUNT(*) AS Total_Shift
FROM Admin A
JOIN Admin_Shift ASH ON A.P_NIK = ASH.Admin_P_NIK
